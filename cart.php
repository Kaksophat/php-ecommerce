...
$total_amount_sql = "SELECT SUM(product_qty * product.price) AS total_amount FROM cart JOIN product ON cart.product_id = product.id";
$result = mysqli_query($conn, $total_amount_sql);
...
if (isset($_POST['delete'])) {
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $qty_sql = "SELECT product_qty FROM cart WHERE product_id = ?";
        $qty_stmt = mysqli_prepare($conn, $qty_sql);
        mysqli_stmt_bind_param($qty_stmt, 'i', $product_id);
        mysqli_stmt_execute($qty_stmt);
        $qty_result = mysqli_stmt_get_result($qty_stmt);
        $qty_row = mysqli_fetch_assoc($qty_result);
        $qty = $qty_row['product_qty'];

        if ($qty == 1) {
            // If quantity is 1, delete the product
            $delete_sql = "DELETE FROM cart WHERE product_id = ?";
            $delete_stmt = mysqli_prepare($conn, $delete_sql);
            mysqli_stmt_bind_param($delete_stmt, 'i', $product_id);
            mysqli_stmt_execute($delete_stmt);
        } else {
            // Decrease the quantity by 1
            $update_sql = "UPDATE cart SET product_qty = product_qty - 1 WHERE product_id = ?";
